...
export const useChatMessage = () => {
  ...
  const fetch = async (params: any, isAdd: boolean = false): Promise<void> => {
    return chatMessageApiClient.fetch(params).then((data: { paginate: Paginate, data: ChatMessageApi[] }) => {
      paginate.value = data.paginate
      if (isAdd) {
        chatMessages.value = [...chatMessages.value, ...data.data.map(convertChatMessageApiToChatMessage)];
      } else {
        chatMessages.value = data.data.map(convertChatMessageApiToChatMessage);
      }
  ...
  const all = async (params: any): Promise<void> => {
    return chatMessageApiClient.all(params).then((data: ChatMessageApi[]) => {
  ...
  const find = async (id: string): Promise<void> => {
    return chatMessageApiClient.find(id).then((data: ChatMessageApi | null) => {
      chatMessage.value = data ? convertChatMessageApiToChatMessage(data) : null;
    }).catch((errorResponse: ErrorResponse) => {
      throw errorResponse
    })
  }
  ...
  const create = async (params: any): Promise<void> => {
    return chatMessageApiClient.create(params).then((data: ChatMessageApi | null) => {
      chatMessage.value = data ? convertChatMessageApiToChatMessage(data) : null;
    }).catch((errorResponse: ErrorResponse) => {
      throw errorResponse
    })
  ...
  const update = async (id: string, params: any): Promise<void> => {
    return chatMessageApiClient.update(id, params).then((data: ChatMessageApi | null) => {
      chatMessage.value = data ? convertChatMessageApiToChatMessage(data) : null;
    }).catch((errorResponse: ErrorResponse) => {
      throw errorResponse
    })
  ...
  const destroy = async (id: string): Promise<void> => {
    return chatMessageApiClient.destroy(id).then(() => {
